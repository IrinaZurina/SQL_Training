DROP DATABASE IF EXISTS gb_sql_homework4;
CREATE DATABASE gb_sql_homework4;
USE gb_sql_homework4;
...
FROM (SELECT p.user_id, m.id, l.media_id FROM profiles p
      ...
      WHERE (DATEDIFF(CURDATE(), birthday) / 365) < 12) as tmp;
...
SELECT COUNT(id)
FROM (SELECT u.id, count(body) AS number_of_messages
	  FROM users u LEFT JOIN messages m ON u.id = m.from_user_id
      GROUP BY u.id
      HAVING number_of_messages= 0) as tmp;
...
FROM friends f LEFT JOIN messages m
