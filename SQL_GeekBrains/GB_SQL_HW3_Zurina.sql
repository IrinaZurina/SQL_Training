...
SELECT COUNT(user_id) `num_of_male_users_35+`
FROM profiles
WHERE (gender = 'm') AND (DATEDIFF(CURDATE(), birthday) / 365 > 35);

-- Сколько заявок в друзья в каждом статусе? (таблица friend_requests) [GROUP BY] --
SELECT `status`, COUNT(initiator_user_id) AS num_of_requests
FROM friend_requests
GROUP BY status;

 -- * Выведите номер пользователя, который отправил больше всех заявок в друзья (таблица friend_requests) [LIMIT]. --
SELECT initiator_user_id, COUNT(target_user_id) AS num_of_requests
FROM friend_requests
