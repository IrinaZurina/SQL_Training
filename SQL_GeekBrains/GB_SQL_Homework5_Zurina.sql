DROP DATABASE IF EXISTS gb_sql_homework5;
CREATE DATABASE gb_sql_homework5;
...
FROM `profiles` p
LEFT JOIN users u ON user_id = id
WHERE DATEDIFF(CURDATE(), birthday) / 365 <= 20);
...
SELECT created_at, body,
	   TIMEDIFF(created_at, LAG(created_at) OVER ()) as `time_diff`
FROM messages
ORDER BY created_at;